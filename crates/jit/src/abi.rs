...
//!
//! ```text
//!   offset 0x00000  register image (REGS_BASE)           regs::offsets::SIZE bytes
//!   offset 0x00340  control block (CTRL_BASE)            exit_reason, exit_pc
//!   offset 0x10000  guest RAM window (RAM_BASE)          ram_bytes
//! ```
//!
//! The register image (the live `CpuState`) lives at [`REGS_BASE`] so generated
//! code can read e.g. `X5` as an
...
//! ```
//!
//! `$regs_base` is the base of the register image (always [`REGS_BASE`] for
...
/// Base of the register image in linear memory.
pub const REGS_BASE: u32 = 0;

/// Size of the register image, in bytes (for allocating the image buffer).
