...
fn xlate<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, write: bool, el: u8) -> Option<u64> {
...
fn gather<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, n: u64, el: u8) -> u128 {
    let mut val = 0u128;
    let mut done = 0u64;
    while done < n {
        let cur = va + done;
...
fn scatter<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, n: u64, val: u128, el: u8) {
    let mut done = 0u64;
    while done < n {
        let cur = va + done;
        let page_end = (cur & !(PAGE - 1)) + PAGE;
...
fn read_at<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, size: u8, el: u8) -> u64 {
    let n = 1u64 << size;
...
fn write_at<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, size: u8, val: u64, el: u8) {
    let n = 1u64 << size;
    if in_one_page(va, n) {
...
pub(crate) fn read<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, size: u8) -> u64 {
    read_at(cpu, mem, va, size, cpu.el)
}

/// Ordinary (privileged-at-current-EL) sized write.
pub(crate) fn write<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, size: u8, val: u64) {
...
pub(crate) fn read_unpriv<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, size: u8) -> u64 {
    read_at(cpu, mem, va, size, 0)
}

/// Unprivileged sized write (STTR): permission-checked as EL0 even from EL1.
pub(crate) fn write_unpriv<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, size: u8, val: u64) {
    write_at(cpu, mem, va, size, val, 0);
}

/// Read `1 << log2` bytes (log2 0..=4) into a u128, for SIMD/FP accesses.
pub(crate) fn read_vec<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, log2: u8) -> u128 {
    gather(cpu, mem, va, 1u64 << log2, cpu.el)
}

/// Write the low `1 << log2` bytes of `val` to memory.
pub(crate) fn write_vec<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, va: u64, log2: u8, val: u128) {
...
pub(crate) fn dc_zva<M: GuestMem>(cpu: &mut CpuState, mem: &mut M, rt: u8) {
