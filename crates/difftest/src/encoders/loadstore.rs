...
pub(super) fn classes() -> Vec<MemClass> {
    vec![
        ...
        MemClass { name: "ldst_vec_reg", encode: ldst_vec_reg },
        MemClass { name: "ldst_vec_pair", encode: ldst_vec_pair },
        MemClass { name: "ldst_struct_multi", encode: ldst_struct_multi },
        MemClass { name: "ldst_struct_single", encode: ldst_struct_single },
    ]
}
...
fn random_v_seeds(rng: &mut Rng) -> Vec<(u8, u128)> {
    (0..32)
        .map(|r| (r, (u128::from(rng.next_u64()) << 64) | u128::from(rng.next_u64())))
        .collect()
}

/// SIMD/FP load/store single register (V=1), unsigned-immediate form, B/H/S/D/Q.
fn ldst_vec_reg(rng: &mut Rng) -> MemEncoded {
    // (size[31:30], opc_hi[23], log2 bytes).
    let (size30, opc_hi, log2) = [(0u32, 0u32, 0u32), (1, 0, 1), (2, 0, 2), (3, 0, 3), (0, 1, 4)]
...
fn ldst_vec_pair(rng: &mut Rng) -> MemEncoded {
    let (opc, vesize) = [(0u32, 2u32), (1, 3), (2, 4)][rng.below(3) as usize]; // S/D/Q
...
fn ldst_struct_multi(rng: &mut Rng) -> MemEncoded {
    let q = rng.below(2);
    // (opcode, selem).
    let (opcode, selem) =
        [(0x0u32, 4u32), (0x2, 1), (0x4, 3), (0x6, 1), (0x7, 1), (0x8, 2), (0xa, 1)]
...
fn ldst_struct_single(rng: &mut Rng) -> MemEncoded {
...
fn ldst_atomic(rng: &mut Rng) -> MemEncoded {
    ...
    MemEncoded { init_v: Vec::new(), word, seeds: vec![(rn as u8, base_near_mid(rng, 0x100))], data: random_data(rng) }
}
